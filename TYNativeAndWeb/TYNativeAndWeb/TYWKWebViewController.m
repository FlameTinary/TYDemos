...
@interface TYWeakWebViewScriptMessageDelegate : NSObject<WKScriptMessageHandler>
...
@end
@implementation TYWeakWebViewScriptMessageDelegate
...
@interface TYWKWebViewController ()<WKScriptMessageHandler, WKUIDelegate, WKNavigationDelegate>
...
-(void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if ([keyPath isEqualToString:NSStringFromSelector(@selector(estimatedProgress))] && object == _webView) {
        ...
        [_webView evaluateJavaScript:@"document.title" completionHandler:^(NSString * title, NSError * _Nullable error) {
...
- (void)localHtmlClicked{
    NSString *path = [[NSBundle mainBundle] pathForResource:@"TYTest" ofType:@"html"];
...
- (WKWebView *)webView{
    ...
    if(_webView == nil){
        ...
        TYWeakWebViewScriptMessageDelegate *weakScriptMessageDelegate = [[TYWeakWebViewScriptMessageDelegate alloc] initWithDelegate:self];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if([message.name isEqualToString:@"jsToOcNoPrams"]){
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"js调用到了oc" message:parameter[@"params2"] preferredStyle:UIAlertControllerStyleAlert];
        [alertController addAction:([UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        }])];
        [self presentViewController:alertController animated:YES completion:nil];
    }
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    NSLog(@"发送跳转请求：%@",urlStr);
    //自己定义的协议头
    NSString *htmlHeadString = @"sheldon://";
    if([urlStr hasPrefix:htmlHeadString]){
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"通过截取URL调用OC" message:@"你是否想返回?" preferredStyle:UIAlertControllerStyleAlert];
